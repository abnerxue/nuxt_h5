<template>
  <div class="integralDetail" id="integralDetail">
    ...
    <div class="buyTab">
      <a
        :href="'https://chat.sobot.com/chat/h5/v2/index.html?sysnum=e772a07377c14e239174ee712efa2647&params='+JSON.stringify({'商品信息网址':'https://www.huainanhui.com/score/detail/'+ id +'.html'})"
      >
        <i class="iconfont icon-kefu"></i>客服
      </a>
      <div class="gobuy" @click="buy()">立即兑换</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    account(str) {
      if (str == "+") {
        if (this.score.quota == this.num) {
          Toast({
            message: "该商品一次只能购买" + this.score.quota + this.goods.unit,
            position: "bottom"
          });
        } else {
          ++this.num;
    ...
    getData() {
      axios
        .get("/ppi/index.php/api/score/detail?scoreId=" + this.id)
        .then(res => {
          this.imgs = res.data.goods.imgs;
          this.goods = res.data.goods;
          this.score = res.data.scoreGoods;
          let norm = res.data.goods.norms;
          this.gid = this.score.gid;
          if (norm) {
    ...
    // 获取滚动条高度
    handleScroll() {
      let b = document.getElementById("integralDetail").scrollTop;
    ...
    recommendGoods() {
      ...
      axios.post("/ppi/index.php/api/goods/recommend").then(res => {
